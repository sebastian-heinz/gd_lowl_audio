...
std::shared_ptr<Lowl::Audio::AudioData> GdLowlAudioData::get_audio_data() const {
    return audio_data;
}

Ref<GdLowlAudioData> GdLowlAudioData::create_slice(double p_begin_sec, double p_end_sec) {
    std::shared_ptr<Lowl::Audio::AudioData> slice = audio_data->create_slice(p_begin_sec, p_end_sec);
...
GdLowlAudioData::GdLowlAudioData(Array p_audio_frames, double p_sample_rate, int p_channel) :
        GdLowlAudioSource(std::make_shared<Lowl::Audio::AudioData>(
                convert_frames(p_audio_frames, p_channel), p_sample_rate, Lowl::Audio::get_channel(p_channel)
        )) {
    audio_data = std::dynamic_pointer_cast<Lowl::Audio::AudioData>(get_audio_source());
}

GdLowlAudioData::GdLowlAudioData(std::shared_ptr<Lowl::Audio::AudioData> p_audio_data) :
...
std::vector<Lowl::Audio::AudioFrame> GdLowlAudioData::convert_frames(Array p_audio_frames, int p_channel) {
    std::vector<Lowl::Audio::AudioFrame> frames = std::vector<Lowl::Audio::AudioFrame>();
    for (int i = 0; i < p_audio_frames.size(); i++) {
        Lowl::Audio::AudioFrame frame;
        Variant v = p_audio_frames[i];
        if (v.get_type() != Variant::PACKED_FLOAT32_ARRAY) {
            continue;
        }
        PackedFloat32Array entry = v;
        for (int j = 0; j < p_channel && j < entry.size() && j < Lowl::Audio::AudioFrame::MAX_CHANNEL; j++) {
            frame[j] = entry[j];
        }
    }
    return frames;
