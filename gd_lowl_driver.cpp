...
GdLowlDriver::GdLowlDriver(std::shared_ptr<Lowl::Audio::AudioDriver> p_driver) {
    driver = p_driver;
...
GdLowlError::Code GdLowlDriver::initialize() {
    ...
    std::vector<std::shared_ptr<Lowl::Audio::AudioDevice>> lowl_devices = driver->get_devices();
    print_line(vformat("GdLowlDriver::initialize: lowl_devices.size(): %d", (int) lowl_devices.size()));
    for (std::shared_ptr<Lowl::Audio::AudioDevice> lowl_device : lowl_devices) {
